...
$follow=$_GET["follow"];
if($follow=="follow"){
	mysqli_query($conn,"insert into `follow` values('{$userid}','{$publicuserid}')")
	or die("SQL Failed");
}elseif($follow=="unfollow"){
	mysqli_query($conn,"delete from `follow` where userid='{$userid}' and followedid='{$publicuserid}'")
	or die("SQL Failed");
}
		...
		<div class="col-md-12 column">
			...
			<dl>
				...
				<dd>
					...
					<?php
					$followornot=mysqli_query($conn,"select * from follow where userid='{$userid}' and followedid='{$publicuserid}'")
					or die("SQL Failed");
					echo "<br>";

					if(mysqli_num_rows($followornot)<=0){
						$follow1="follow";
						echo "<a href='publicprofile.php?userid={$publicuserid}&follow={$follow1}' class='btn btn-info btn-sm' role='button'>Follow</a>";
					}else{
						$follow1="unfollow";
						echo "<a href='publicprofile.php?userid={$publicuserid}&follow={$follow1}' class='btn btn-info btn-sm' role='button'>Unfollow</a>";
					}
	...
	<div class="row clearfix">
		<div class="col-md-4 column">
			...
			<?php
				...
				$userfollows=mysqli_query($conn,"select userid,username from user natural join (select followedid as userid from follow natural join user where userid='{$publicuserid}')as followed");
		...
		<div class="col-md-4 column">
			...
			<?php
				...
				$userlikes=mysqli_query($conn,"select projid,projname,userid,username from user natural join (select projid,projname,project.userid from project join `likes` using (projid) where `likes`.userid='{$publicuserid}')as likess");
		...
		<div class="col-md-4 column">
			...
			<p>
			<?php
			$userprojects=mysqli_query($conn,"select projname, projid from project where userid='{$publicuserid}'");
